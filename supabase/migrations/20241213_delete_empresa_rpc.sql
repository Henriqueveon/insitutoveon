...
DECLARE
    ...
    v_empresa_nome TEXT;
BEGIN
    -- Buscar nome da empresa antes de deletar
    ...
    -- Transações
    DELETE FROM public.transacoes_recrutamento WHERE empresa_id = p_empresa_id;
    ...
    -- Deletar a empresa
    DELETE FROM public.empresas_recrutamento
    WHERE id = p_empresa_id;
    ...
    ELSE
        RETURN jsonb_build_object(
            'success', false,
            'error', 'Falha ao deletar empresa'
        );
    END IF;
...
GRANT EXECUTE ON FUNCTION public.delete_empresa(UUID) TO anon;
GRANT EXECUTE ON FUNCTION public.delete_empresa(UUID) TO service_role;

-- =====================================================
-- POLÍTICA RLS PARA PERMITIR DELETE EM empresas_recrutamento
-- (Como backup, caso alguém use delete direto)
-- =====================================================
